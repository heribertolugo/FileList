...
using Implementation.Structures;
...
namespace Implementation.Media
{
    internal class BasicMedia : DisposableBase, IMedia, INativePointer, IReferenceCount, IEventProvider
    {
        ...
        private SlaveMediaCollection m_slaves;
            ...
            get
            {
                unsafe
                {
                    ...
                    for (int i = 0; i < num; i++)
                    {
                        ...
                        track.iCodec = pTrackInfo->i_codec;
        ...
        public MediaType MediaType
        {
            get 
            {
                return (MediaType)(int)LibVlcMethods.libvlc_media_get_type(m_hMedia);
            }
        }

        public ISlaveMediaCollection Slaves
        {
            get
            {
                if(m_slaves == null)
                {
                    m_slaves = new SlaveMediaCollection(m_hMedia);
                }
        ...
        public MediaParseStatus MediaParseStatus
        {
            get
            {
                return (MediaParseStatus)LibVlcMethods.libvlc_media_get_parsed_status(m_hMedia);
            }
        }

        public void ParseAsyncWithOptions(MediaParseType parseFlags, int timeout)
        {
            int res = LibVlcMethods.libvlc_media_parse_with_options(m_hMedia, (libvlc_media_parse_flag_t)parseFlags, timeout);
            if(res == -1)
            {
                throw new LibVlcException("Failed to parse media with options");
            }
        }

        public void ParseStop()
        {
            LibVlcMethods.libvlc_media_parse_stop(m_hMedia);
        }
        ...
        internal virtual void OnPlayerMediaSet()
        {
