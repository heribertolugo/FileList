    ...
    internal class VideoPlayer : AudioPlayer, IVideoPlayer
    {
        ...
        public float PlaybackRate
        {
            ...
            set
            {
                ...
                if (res == -1)
                {
                    throw new LibVlcException("Failed to set playback rate");
                }
            }
        }
        ...
        void Events_PlayerPlaying(object sender, EventArgs e)
        {
            try
            {
                var tracksInfo = CurrentMedia.TracksInfoEx;
        ...
        public void AddSlaveMedia(SlaveMedia slave)
        {
            int res = LibVlcMethods.libvlc_media_player_add_slave(m_hMediaPlayer, (libvlc_media_slave_type_t)slave.SlaveType,
                slave.Url.ToUtf8(), true);
            if (res == -1)
            {
                throw new LibVlcException("Failed to add slave media");
            }
        }
    }
}
